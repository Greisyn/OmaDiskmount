...
// oShelf-style action pill. Ink always comes from the theme.
Rectangle {
  id: root
  ...
  Row {
    ...
    DriveGlyph { visible: !!root.icon; kind: root.icon; width: root.icon ? 20 : 0; height: 20; ink: root.selected ? Color.accent : Color.foreground }
    Text {
      id: caption
      visible: root.label !== ""
